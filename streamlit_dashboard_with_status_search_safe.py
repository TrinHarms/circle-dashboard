...
with tab1:
    ...
    damage_summary = df_view['Damage Types'].value_counts().reset_index().rename(columns={'index': 'ประเภทความเสียหาย', 'Damage Types': 'จำนวนที่พบ'})
    if not damage_summary.empty:
        fig = px.bar(damage_summary, x='จำนวนที่พบ', y='ประเภทความเสียหาย', orientation='h')
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("ไม่มีข้อมูลความเสียหายที่ตรงกับเงื่อนไขที่เลือก")
